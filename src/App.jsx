...
import { 
  Map, CheckSquare, Wallet, MessageCircle, ChevronDown, ChevronUp, 
  Plus, Trash2, Plane, Hotel, Car, Calendar, ExternalLink, MapPin, 
  X, Music, Camera, Coffee, Train, ArrowRight, ShoppingBag, 
  CloudSnow, Languages, CreditCard, Banknote, User, Search, Heart, Star, Edit2, Save, Copy, Check, Luggage, Users, ArrowLeft, UserPlus, MinusCircle, Upload, Image as ImageIcon, ChevronLeft, ChevronRight, Globe
} from 'lucide-react';
...
const openFlight = (flightNo) => {
  window.open(`https://www.google.com/search?q=${flightNo}+flight+status`, '_blank');
};

const openTranslate = (source) => {
  const sl = source === 'cn' ? 'zh-CN' : 'en';
  window.open(`https://translate.google.com/?sl=${sl}&tl=ja&op=translate`, '_blank');
};
...
const INITIAL_TRANSFERS = [
  { id: 1, date: '12/25', from: 'New Chitose', to: 'Tomamu', note: 'Arrival Pickup', location: 'New Chitose Airport' },
  { id: 2, date: '12/27', from: 'Tomamu', to: 'Asahikawa', note: 'Sightseeing Charter', location: 'Shirahige Falls' },
  { id: 3, date: '12/28', from: 'Asahikawa', to: 'Otaru', note: 'Via Tenguyama', location: 'Otaru Tenguyama Ropeway' },
  { id: 4, date: '12/29', from: 'Otaru', to: 'Sapporo', note: 'City Transfer', location: 'Sapporo' },
  { id: 5, date: '01/01', from: 'Sapporo', to: 'New Chitose', note: 'Departure', location: 'New Chitose Airport' },
];
...
const WINTER_PHRASES = [
  { jp: 'リフト乗り場はどこですか？', romaji: 'Rifuto noriba wa doko desu ka?', cn: '缆车在哪?' },
  { jp: '寒いです', romaji: 'Samui desu', cn: '好冷' },
  { jp: 'スキーをレンタルしたいです', romaji: 'Sukii o rentaru shitai desu', cn: '我想租滑雪板' },
  { jp: 'トイレはどこですか？', romaji: 'Toire wa doko desu ka?', cn: '厕所在哪?' },
...
const PlanTab = () => {
  ...
  const handleNoteChange = (day, note) => {
    const updated = itinerary.map(item => item.day === day ? { ...item, note: note } : item);
    setItinerary(updated);
  };
  const startEditing = (day) => { setIsEditing(true); setEditingDay(day); setExpandedDay(day); };
  const saveEditing = () => { setIsEditing(false); setEditingDay(null); };
  ...
  const deleteActivity = (day, idx) => {
    const updated = itinerary.map(item => {
      if (item.day === day) {
        return { ...item, activities: item.activities.filter((_, i) => i !== idx) };
      }
      return item;
    });
    setItinerary(updated);
  ...
  const addActivity = (day) => {
    const updated = itinerary.map(item => {
      if (item.day === day) {
        return { ...item, activities: [...item.activities, { time: '00:00', content: 'New Activity', location: '' }] };
      }
      return item;
    });
    setItinerary(updated);
  ...
  const updateTransfer = (id, field, value) => {
      const updated = transfers.map(t => t.id === id ? { ...t, [field]: value } : t);
      setTransfers(updated);
  };
  const deleteTransfer = (id) => setTransfers(transfers.filter(t => t.id !== id));
  const addTransfer = () => setTransfers([...transfers, { id: Date.now(), date: '12/xx', from: 'Origin', to: 'Dest', note: 'Note', location: '' }]);

  const updateAccommodation = (id, field, value) => {
      const updated = accommodations.map(h => h.id === id ? { ...h, [field]: value } : h);
  ...
  const addRoomDetail = (hotelId) => {
      const updated = accommodations.map(h => {
          if (h.id === hotelId) {
              return { ...h, details: [...h.details, { room: '', ref: '', source: '' }] };
          }
          return h;
      });
      setAccommodations(updated);
  ...
  const getTabClass = (name) => 
    `px-4 py-2 rounded-full text-sm font-semibold transition-all whitespace-nowrap ${subTab === name ? 'bg-[#222222] text-white shadow-md' : 'bg-white border border-[#DDDDDD] text-[#717171] hover:border-[#222222] hover:text-[#222222]'}`;

  return (
    <div className="h-full flex flex-col pb-24 bg-[#F7F7F7]">
      <div className="bg-white sticky top-0 z-20 px-6 py-4 shadow-sm">
        <div className="flex gap-2 overflow-x-auto no-scrollbar">
          <button onClick={() => setSubTab('schedule')} className={getTabClass('schedule')}>Itinerary</button>
          <button onClick={() => setSubTab('flight')} className={getTabClass('flight')}>Flights</button>
          <button onClick={() => setSubTab('hotel')} className={getTabClass('hotel')}>Stays</button>
          <button onClick={() => setSubTab('transfer')} className={getTabClass('transfer')}>Transfers</button>
        </div>
      </div>

      <div className="flex-1 px-6 pt-4 overflow-y-auto space-y-4 pb-20">
        {/* Schedule */}
        {subTab === 'schedule' && itinerary.map((item) => {
          const isExpanded = expandedDay === item.day;
          const displayMonth = item.date.includes('Jan') ? 'JAN' : 'DEC';
          
          return (
            <div key={item.day} className="bg-white rounded-2xl border border-[#DDDDDD] overflow-hidden hover:shadow-lg transition-shadow duration-300 relative">
              <button
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="flex flex-col items-center justify-center w-12 h-12 bg-[#F7F7F7] rounded-xl border border-[#DDDDDD]">
                    <span className="text-[10px] uppercase font-bold text-[#717171]">{displayMonth}</span>
                    <span className="text-lg font-extrabold text-[#222222]">{item.date.split(' ')[1]}</span>
                  </div>
                  <div className="text-left">
                    <h3 className="font-bold text-[#222222] text-base">{item.titleCn}</h3>
                ...
                <div className="flex flex-col items-end gap-1">
                    ...
                    {item.weather && (
                        <div className="flex items-center gap-1 bg-[#F7F7F7] px-2 py-0.5 rounded-full border border-[#DDDDDD]">
                            <span className="text-[#717171]"><CloudSnow size={10}/></span>
                            <span className="text-[10px] font-bold text-[#222222]">{item.weather}</span>
                        </div>
                    )}
                </div>
              </button>
                ...
                <div className="px-5 pb-5 animate-fadeIn">
                  <div className="flex justify-end mb-2">
                      {isEditing && editingDay === item.day ? (
                          <button onClick={saveEditing} className="flex items-center gap-1 text-xs font-bold text-white bg-green-500 px-3 py-1.5 rounded-full"><Save size={12}/> Done</button>
                      ) : (
                          <button onClick={() => startEditing(item.day)} className="flex items-center gap-1 text-xs font-bold text-[#717171] hover:text-[#FF5A5F] bg-[#F7F7F7] px-3 py-1.5 rounded-full"><Edit2 size={12}/> Edit</button>
                      )}
                  </div>
                  ...
                  <div className="space-y-6 pl-4 border-l border-[#DDDDDD] ml-6 py-2">
                        ...
                        {isEditing && editingDay === item.day ? (
                            <div className="flex gap-2 items-start bg-[#F7F7F7] p-2 rounded-lg">
                                <div className="flex-1 space-y-2">
                                    <input value={act.time} onChange={(e) => updateActivity(item.day, idx, 'time', e.target.value)} className="w-full text-xs font-bold p-1 rounded border border-[#DDDDDD]" />
                                    <textarea value={act.content} onChange={(e) => updateActivity(item.day, idx, 'content', e.target.value)} className="w-full text-sm p-1 rounded border border-[#DDDDDD] resize-none" rows="2" />
                                </div>
                                <button onClick={() => deleteActivity(item.day, idx)} className="text-red-400 p-1"><Trash2 size={16}/></button>
                            </div>
                        ) : (
                            <div className="flex justify-between items-start">
                                <div className="flex-1 pr-2">
                                    <span className="inline-block text-xs font-bold text-[#222222] mb-0.5 bg-[#F7F7F7] px-1.5 py-0.5 rounded mr-2">{act.time}</span>
                                    <span className="text-sm text-[#222222] leading-relaxed">{act.content}</span>
                                </div>
                                <button onClick={(e) => { e.stopPropagation(); openMap(act.location); }} className="text-[#717171] hover:text-[#FF5A5F] transition-colors p-1"><MapPin size={18} /></button>
                            </div>
                        )}
                      </div>
                    ))}
                    ...
                    {isEditing && editingDay === item.day && (
                        <button onClick={() => addActivity(item.day)} className="flex items-center gap-1 text-xs font-bold text-[#FF5A5F] mt-2 ml-1"><Plus size={14}/> Add Activity</button>
                    )}
                  </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-[#DDDDDD]">
                    <textarea 
                        ...
                        rows="2" placeholder="Add a private note for this day..."
                        value={item.note || ''} onChange={(e) => handleNoteChange(item.day, e.target.value)}
                    />
                  </div>
                </div>
              )}
        ...
        {subTab === 'flight' && (
            <div className="space-y-4">
                <div className="bg-[#FF5A5F]/10 p-4 rounded-xl border border-[#FF5A5F] flex items-center justify-between">
                     <div>
                        <span className="text-xs font-bold text-[#FF5A5F] uppercase">Trip.com Booking ID</span>
                        <div className="text-lg font-mono font-bold text-[#222222]">1433808123136448</div>
                     </div>
                     <button onClick={() => copyToClipboard('1433808123136448')} className="bg-white p-2 rounded-full text-[#FF5A5F] shadow-sm"><Copy size={16}/></button>
                </div>

                {INITIAL_FLIGHTS.map((f, i) => (
                    <div key={i} className="bg-white p-6 rounded-2xl border border-[#DDDDDD] shadow-sm relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-full h-1 bg-[#FF5A5F]"></div>
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <span className="text-xs font-bold text-[#717171] uppercase tracking-wider">{f.type} • {f.airline}</span>
                                <h3 className="text-2xl font-bold text-[#222222] mt-1">{f.route}</h3>
                                <div className="text-sm font-medium text-[#FF5A5F] mt-1 flex items-center gap-2">
                                    Booking Ref: {f.ref}
                                    <button onClick={() => copyToClipboard(f.ref)} className="text-[#717171] hover:text-[#222222]"><Copy size={12}/></button>
                                </div>
                            </div>
                            <div className="bg-[#F7F7F7] p-2 rounded-full"><Plane size={24} className="text-[#222222]"/></div>
                        </div>
                        <div className="flex flex-col gap-3">
                            <div className="flex justify-between items-center py-2 border-b border-[#DDDDDD]">
                                <div className="flex items-center gap-2" onClick={() => openFlight(f.flightNo)}>
                                    <span className="text-lg font-semibold text-[#222222] underline decoration-[#DDDDDD] underline-offset-4 cursor-pointer hover:text-[#FF5A5F]">{f.flightNo}</span>
                                    <ExternalLink size={14} className="text-[#717171]"/>
                                </div>
                                <span className="text-sm font-medium text-[#222222]">{f.date}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm text-[#717171]">
                             ...
                             <div className="flex items-center gap-2 text-xs text-[#222222] bg-[#F7F7F7] p-2 rounded-lg mt-2">
                                <Luggage size={14}/> 
                                <span><b>7kg</b> Hand Carry / <b>20kg</b> Check-in</span>
                            </div>
                        </div>
                    </div>
                ))}
        ...
        {subTab === 'hotel' && (
            <div className="space-y-4">
                <div className="flex justify-end">
                    {isEditingStays ? (
                        <button onClick={() => setIsEditingStays(false)} className="bg-green-500 text-white text-xs px-3 py-1.5 rounded-full font-bold flex items-center gap-1"><Save size={12}/> Done</button>
                    ) : (
                        <button onClick={() => setIsEditingStays(true)} className="bg-[#222222] text-white text-xs px-3 py-1.5 rounded-full font-bold flex items-center gap-1"><Edit2 size={12}/> Edit Stays</button>
                    )}
                </div>

                {accommodations.map((hotel) => (
                    <div key={hotel.id} className="bg-white rounded-2xl border border-[#DDDDDD] shadow-sm overflow-hidden group">
                        <div className="relative h-48 bg-gray-200 overflow-hidden">
                            ...
                            {isEditingStays && (
                                <label className="absolute inset-0 bg-black/40 flex items-center justify-center cursor-pointer transition-opacity">
                                    ...
                                    <input type="file" accept="image/*" className="hidden" onChange={(e) => handleHotelImageUpload(hotel.id, e)} />
                                </label>
                            )}
                            ...
                            {!isEditingStays && (
                                <>
                                    <div className="absolute top-3 right-3 bg-white px-2 py-1 rounded-md shadow-md flex items-center gap-1">
                                        <Star size={12} className="fill-[#FF5A5F] text-[#FF5A5F]"/>
                                        <span className="text-xs font-bold text-[#222222]">{hotel.rating}</span>
                                    </div>
                        ...
                        <div className="p-5">
                            {isEditingStays ? (
                                <div className="space-y-3">
                                    <input className="w-full font-bold text-lg border-b border-[#DDDDDD] outline-none" value={hotel.name} onChange={(e) => updateAccommodation(hotel.id, 'name', e.target.value)} />
                                    <input className="w-full text-sm text-[#717171] border-b border-[#DDDDDD] outline-none" value={hotel.nameCn} onChange={(e) => updateAccommodation(hotel.id, 'nameCn', e.target.value)} />
                                    <div className="flex gap-2">
                                        <input className="w-1/2 text-xs border rounded p-1" value={hotel.dates} onChange={(e) => updateAccommodation(hotel.id, 'dates', e.target.value)} />
                                        <input className="w-1/4 text-xs border rounded p-1" value={hotel.rating} onChange={(e) => updateAccommodation(hotel.id, 'rating', e.target.value)} placeholder="Rating" />
                                    </div>
                                    
                                    {/* Editable Details List */}
                                    <div className="space-y-2 border-t border-[#DDDDDD] pt-2 mt-2">
                                        <label className="text-xs font-bold text-[#717171]">Rooms & Booking Refs:</label>
                                        {hotel.details.map((detail, idx) => (
                                            <div key={idx} className="flex gap-2 items-center">
                                                <input className="w-1/3 text-xs border p-1 rounded" placeholder="Room" value={detail.room} onChange={(e) => updateRoomDetail(hotel.id, idx, 'room', e.target.value)} />
                                                <input className="w-1/4 text-xs border p-1 rounded" placeholder="Source" value={detail.source} onChange={(e) => updateRoomDetail(hotel.id, idx, 'source', e.target.value)} />
                                                <input className="flex-1 text-xs border p-1 rounded" placeholder="Ref" value={detail.ref} onChange={(e) => updateRoomDetail(hotel.id, idx, 'ref', e.target.value)} />
                                                <button onClick={() => deleteRoomDetail(hotel.id, idx)} className="text-red-400"><Trash2 size={14}/></button>
                                            </div>
                                        ))}
                                        <button onClick={() => addRoomDetail(hotel.id)} className="text-xs text-[#FF5A5F] flex items-center gap-1"><Plus size={12}/> Add Room</button>
                                    </div>

                                    <button onClick={() => deleteAccommodation(hotel.id)} className="text-red-500 text-xs font-bold flex items-center gap-1 mt-4"><Trash2 size={12}/> Delete Hotel</button>
                                </div>
                            ) : (
                                <>
                ...
                {isEditingStays && (
                    <button onClick={addAccommodation} className="w-full py-3 border-2 border-dashed border-[#DDDDDD] rounded-2xl text-[#717171] font-bold text-sm hover:border-[#FF5A5F] hover:text-[#FF5A5F] flex justify-center items-center gap-2">
        ...
        {subTab === 'transfer' && (
            <div className="space-y-4">
                 <div className="flex justify-end">
                    {isEditingTransfer ? (
                        <button onClick={() => setIsEditingTransfer(false)} className="bg-green-500 text-white text-xs px-3 py-1.5 rounded-full font-bold flex items-center gap-1"><Save size={12}/> Done</button>
                    ) : (
                        <button onClick={() => setIsEditingTransfer(true)} className="bg-[#222222] text-white text-xs px-3 py-1.5 rounded-full font-bold flex items-center gap-1"><Edit2 size={12}/> Edit</button>
                    )}
                 </div>

                {transfers.map((t) => (
                    <div key={t.id} className="bg-white rounded-2xl p-5 border border-[#DDDDDD] shadow-sm relative hover:border-[#FF5A5F] transition group">
                        {isEditingTransfer ? (
                             <div className="space-y-3">
                                <div className="flex gap-2">
                                    <input value={t.date} onChange={(e) => updateTransfer(t.id, 'date', e.target.value)} className="w-16 p-1 border rounded text-xs font-bold text-center" />
                                    <input value={t.from} onChange={(e) => updateTransfer(t.id, 'from', e.target.value)} className="flex-1 p-1 border rounded text-sm" placeholder="From" />
                                    <span className="self-center">➔</span>
                                    <input value={t.to} onChange={(e) => updateTransfer(t.id, 'to', e.target.value)} className="flex-1 p-1 border rounded text-sm" placeholder="To" />
                                </div>
                                <input value={t.note} onChange={(e) => updateTransfer(t.id, 'note', e.target.value)} className="w-full p-1 border rounded text-xs text-gray-500" placeholder="Note" />
                                <button onClick={() => deleteTransfer(t.id)} className="absolute -top-2 -right-2 bg-red-500 text-white p-1 rounded-full shadow-md"><X size={14}/></button>
                             </div>
                        ) : (
                            <div className="flex items-center gap-4 cursor-pointer" onClick={() => openMap(t.location)}>
                                <div className="flex flex-col items-center justify-center min-w-[50px]">
                                    <span className="text-xs font-bold text-[#717171] uppercase">Dec</span>
                                    <span className="text-xl font-extrabold text-[#222222]">{t.date.split('/')[1]}</span>
                                </div>
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex justify-between items-center mt-2">
                                        <span className="text-xs bg-[#F7F7F7] text-[#717171] px-2 py-1 rounded-md font-medium border border-[#DDDDDD]">{t.note}</span>
                ...
                {isEditingTransfer && (
                    <button onClick={addTransfer} className="w-full py-3 border-2 border-dashed border-[#DDDDDD] rounded-2xl text-[#717171] font-bold text-sm hover:border-[#FF5A5F] hover:text-[#FF5A5F] flex justify-center items-center gap-2">
...
const ChecklistTab = () => {
  ...
  if (activeUser) {
      ...
      const toggleItem = (id) => {
        const updatedUserList = currentList.map(i => i.id === id ? { ...i, completed: !i.completed } : i);
      ...
      const addItem = (e) => {
        ...
        setChecklists({ ...checklists, [activeUser]: [...currentList, newItemObj] });
        setNewItem('');
      };
      ...
      return (
        <div className="flex flex-col h-full pb-24 bg-[#F7F7F7]">
            <div className="bg-white sticky top-0 z-20 px-6 py-4 shadow-sm border-b border-[#DDDDDD] flex items-center gap-3">
                <button onClick={() => setActiveUser(null)} className="p-2 -ml-2 rounded-full hover:bg-gray-100 transition-colors">
                ...
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full border border-[#DDDDDD] overflow-hidden bg-[#F7F7F7] flex-shrink-0">
                        {avatarSrc ? (
                            <img src={avatarSrc} alt={activeUser} className="w-full h-full object-cover" />
                        ) : (
                    ...
                    <div>
                        <h2 className="text-lg font-extrabold text-[#222222] leading-none">{activeUser}</h2>
            ...
            <div className="flex-1 px-6 pt-6 overflow-y-auto">
                <div className="bg-white p-5 rounded-2xl border border-[#DDDDDD] shadow-sm mb-4 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-[#717171] text-xs mt-1">{currentList.length - currentList.filter(i => i.completed).length} items left</p>
                    </div>
                    <div className="relative w-12 h-12 flex items-center justify-center">
                        <svg className="transform -rotate-90 w-12 h-12">
                            <circle cx="24" cy="24" r="18" stroke="#F7F7F7" strokeWidth="4" fill="transparent" />
                            <circle cx="24" cy="24" r="18" stroke="#FF5A5F" strokeWidth="4" fill="transparent" strokeDasharray={113} strokeDashoffset={113 - (113 * progress) / 100} strokeLinecap="round"/>
                        </svg>
                        <span className="absolute text-[10px] font-bold text-[#222222]">{progress}%</span>
                    </div>
                </div>

                <div className="space-y-3 pb-4">
                    {currentList.map((item) => (
                    <div key={item.id} className={`flex items-center justify-between p-3 rounded-xl border transition-all ${item.completed ? 'bg-[#F7F7F7] border-transparent' : 'bg-white border-[#DDDDDD] shadow-sm'}`}>
                        <div className="flex items-center gap-3 flex-1 cursor-pointer" onClick={() => toggleItem(item.id)}>
                        <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${item.completed ? 'bg-[#FF5A5F] border-[#FF5A5F]' : 'border-[#DDDDDD] bg-white'}`}>
                            {item.completed && <CheckSquare size={12} className="text-white" />}
                        </div>
                        <span className={`text-sm font-medium ${item.completed ? 'line-through text-[#DDDDDD]' : 'text-[#222222]'}`}>{item.text}</span>
                        </div>
                        <button onClick={() => deleteItem(item.id)} className="text-[#DDDDDD] hover:text-[#FF5A5F] transition-colors"><Trash2 size={14} /></button>
                    </div>
                    ))}
                </div>

                <form onSubmit={addItem} className="relative mt-2 mb-24">
                    <input type="text" placeholder="Add item..." className="w-full pl-4 pr-12 py-3 rounded-full border border-[#DDDDDD] bg-white shadow-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#222222]" value={newItem} onChange={(e) => setNewItem(e.target.value)} />
                    <button type="submit" className="absolute right-2 top-2 bottom-2 w-8 bg-[#FF5A5F] rounded-full flex items-center justify-center text-white hover:bg-[#E00B41] transition"><Plus size={16} /></button>
                </form>
            </div>
        </div>
      );
  ...
  return (
    <div className="flex flex-col h-full pb-24 bg-[#F7F7F7]">
        <div className="bg-white p-6 pb-4 border-b border-[#DDDDDD]">
            ...
            <p className="text-[#717171] text-sm mt-1">Select your profile to manage packing.</p>
        </div>
        ...
        <div className="flex-1 px-6 pt-6 overflow-y-auto">
            <div className="flex justify-end mb-4">
                <button 
                    ...
                    className={`text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1 transition-colors ${isEditingUsers ? 'bg-[#FF5A5F] text-white' : 'bg-white border border-[#DDDDDD] text-[#717171]'}`}
                >
                    {isEditingUsers ? <Check size={14}/> : <Edit2 size={14}/>} {isEditingUsers ? 'Done' : 'Edit Users'}
                </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                    ...
                    return (
                        <div key={user} className="relative group">
                            ...
                            <label 
                                ...
                                className={`w-full bg-white p-5 rounded-2xl border border-[#DDDDDD] shadow-sm flex flex-col items-center gap-3 transition-all cursor-pointer ${isEditingUsers ? 'border-dashed border-2 hover:border-[#FF5A5F]' : 'hover:scale-105 hover:shadow-md active:scale-95'}`}
                            >
                                <div className={`w-16 h-16 rounded-full flex items-center justify-center overflow-hidden border-2 relative ${avatarSrc ? 'border-transparent' : (isEditingUsers ? 'bg-[#F7F7F7] text-[#717171] border-[#DDDDDD]' : 'bg-[#F7F7F7] text-[#FF5A5F] border-white shadow-sm')}`}>
                                    {avatarSrc ? (
                                        <img src={avatarSrc} alt={user} className="w-full h-full object-cover" />
                                    ) : (
                                        <span className="text-2xl font-bold">{user.charAt(0)}</span>
                                    )}
                                ...
                                <span className="font-bold text-[#222222] text-sm">{user}</span>
                            </label>
                ...
                {isEditingUsers && (
                    <div className="bg-white p-4 rounded-2xl border-2 border-dashed border-[#DDDDDD] flex flex-col items-center justify-center gap-2">
                        <form onSubmit={addUser} className="w-full flex flex-col items-center gap-2">
                            <input 
                                type="text" 
                                placeholder="Name" 
                            ...
                            />
                            <button type="submit" className="bg-[#222222] text-white p-2 rounded-full hover:scale-110 transition">
                                <Plus size={16}/>
                            </button>
                        </form>
                    </div>
    ...
    return (
        <div className="h-full flex flex-col pb-24 px-6 pt-6 bg-[#F7F7F7]">
             <div className="bg-white p-5 rounded-2xl border border-[#DDDDDD] shadow-sm mb-6">
                ...
                <p className="text-[#717171] text-sm mt-1">Don't forget the souvenirs!</p>
            </div>

            <div className="flex-1 overflow-y-auto grid grid-cols-2 gap-4 content-start pb-4">
                {shopItems.map(item => (
                    <div key={item.id} className="bg-white rounded-2xl border border-[#DDDDDD] overflow-hidden shadow-sm flex flex-col">
                        ...
                        <div className="p-3 flex justify-between items-center bg-white border-t border-[#F7F7F7]">
                            <span className="text-sm font-bold text-[#222222] truncate">{item.text}</span>
                            <button onClick={() => deleteItem(item.id)} className="text-[#DDDDDD] hover:text-[#FF5A5F]"><Trash2 size={14}/></button>
                        </div>
                    </div>
                ))}
                ...
                <div className="aspect-square rounded-2xl border-2 border-dashed border-[#DDDDDD] flex flex-col items-center justify-center bg-white hover:border-[#FF5A5F] transition-colors p-4">
                    <form onSubmit={addItem} className="w-full flex flex-col items-center gap-2">
                        <input type="text" placeholder="Add item..." className="w-full text-center text-sm font-semibold outline-none placeholder-[#DDDDDD]" value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addItem(e)}/>
                        <button type="submit" className="bg-[#222222] text-white p-2 rounded-full hover:scale-110 transition-transform"><Plus size={20}/></button>
                    </form>
                </div>
            </div>
        </div>
...
const BudgetTab = () => {
  ...
  const add = (e) => {
    ...
    setExpenses([{ id: Date.now(), item, amount: parseFloat(amount), payer, method }, ...expenses]);
    setItem(''); setAmount('');
  };
  ...
  const startEditing = (ex) => {
    ...
    setEditPayer(ex.payer);
  };

  const saveEdit = (id) => {
    const updated = expenses.map(e => e.id === id ? { ...e, item: editItem, amount: parseFloat(editAmount), payer: editPayer } : e);
    setExpenses(updated);
    setEditingId(null);
  };
    ...
    return (
      <div className="h-full px-6 pt-6 pb-24 flex flex-col bg-[#F7F7F7]">
        <div 
            ...
            className="bg-[#FF5A5F] rounded-2xl p-6 text-white shadow-lg mb-6 relative overflow-hidden cursor-pointer hover:scale-[1.02] transition-transform active:scale-95"
        >
            <div className="relative z-10 flex justify-between items-center">
                <div>
                    <p className="text-white/80 text-sm font-medium mb-1">Total Trip Expenses</p>
                    <div className="flex items-baseline gap-2">
                        <span className="text-4xl font-extrabold tracking-tight">¥{totalJPY.toLocaleString()}</span>
                    </div>
                    <div className="mt-2 flex items-center gap-2 text-white/60 text-xs">
                        <span>Click for history</span> <ChevronRight size={14}/>
                    </div>
                </div>
                <Wallet size={48} className="text-white/20"/>
            </div>
            <div className="mt-4 pt-4 border-t border-white/20 flex justify-between items-center">
                <span className="text-sm font-medium">≈ MYR {(totalJPY * EXCHANGE_RATE).toFixed(2)}</span>
                <span className="text-xs bg-white/20 px-2 py-1 rounded">Rate: {EXCHANGE_RATE}</span>
            </div>
        </div>

        <div className="bg-white rounded-2xl border border-[#DDDDDD] p-5 mb-6 shadow-sm">
            <h3 className="text-sm font-bold text-[#222222] mb-4 uppercase tracking-wide">Quick Add</h3>
            <form onSubmit={add} className="space-y-4">
                <div className="grid grid-cols-3 gap-3">
                    <div className="col-span-2">
                        <label className="text-xs font-bold text-[#717171] mb-1 block">Item</label>
                        <input type="text" placeholder="e.g. Ramen" className="w-full p-2 bg-[#F7F7F7] rounded-lg border border-transparent focus:bg-white focus:border-[#222222] outline-none text-sm transition-all" value={item} onChange={(e) => setItem(e.target.value)} />
                    </div>
                    <div>
                        <label className="text-xs font-bold text-[#717171] mb-1 block">JPY</label>
                        <input type="number" placeholder="0" className="w-full p-2 bg-[#F7F7F7] rounded-lg border border-transparent focus:bg-white focus:border-[#222222] outline-none text-sm transition-all" value={amount} onChange={(e) => setAmount(e.target.value)} />
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                    <div>
                        <label className="text-xs font-bold text-[#717171] mb-1 block">Payer</label>
                        <select className="w-full p-2 bg-[#F7F7F7] rounded-lg border border-transparent outline-none text-sm text-[#222222] appearance-none" value={payer} onChange={(e) => setPayer(e.target.value)}>
                            {DEFAULT_PAYERS.map(p => <option key={p} value={p}>{p}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="text-xs font-bold text-[#717171] mb-1 block">Method</label>
                        <select className="w-full p-2 bg-[#F7F7F7] rounded-lg border border-transparent outline-none text-sm text-[#222222] appearance-none" value={method} onChange={(e) => setMethod(e.target.value)}>
                            <option value="Credit Card">Credit Card</option>
                            <option value="Cash">Cash</option>
                        </select>
                    </div>
                </div>
                <button type="submit" className="w-full bg-[#222222] text-white py-3 rounded-lg text-sm font-bold hover:opacity-90 transition">Record Expense</button>
            </form>
        </div>

        <div className="flex-1 overflow-y-auto pb-4">
            <div className="flex justify-between items-center mb-3 px-1">
                <h3 className="text-xs font-bold text-[#717171] uppercase">Recent</h3>
                <button onClick={() => setView('details')} className="text-xs text-[#FF5A5F] font-bold">View All</button>
            </div>
            <div className="space-y-3">
                {expenses.slice(0, 3).map((ex) => (
                    <div key={ex.id} className="p-4 bg-white rounded-xl border border-[#DDDDDD] shadow-sm flex justify-between items-center">
                        <div>
                            <span className="block text-[#222222] font-bold text-sm">{ex.item}</span>
                            <div className="flex items-center gap-2 mt-1">
                                <span className="text-[10px] bg-[#F7F7F7] text-[#717171] px-1.5 py-0.5 rounded border border-[#DDDDDD] flex items-center gap-1"><User size={8}/> {ex.payer}</span>
                            </div>
                        </div>
  ...
  if (view === 'details') {
      return (
        <div className="h-full flex flex-col bg-[#F7F7F7] pb-24">
             <div className="bg-white sticky top-0 z-20 px-6 py-4 shadow-sm border-b border-[#DDDDDD] flex items-center gap-3">
                <button onClick={() => setView('summary')} className="p-2 -ml-2 rounded-full hover:bg-gray-100 transition-colors">
                    <ArrowLeft size={20} className="text-[#222222]"/>
                </button>
                <div>
                    <h2 className="text-lg font-extrabold text-[#222222]">Transaction History</h2>
                    <p className="text-xs text-[#717171]">Grouped by date</p>
                </div>
            </div>
                                    ...
                                    {editingId === ex.id ? (
                                        <div className="flex gap-2 items-center">
                                            <div className="flex-1 space-y-2">
                                                ...
                                                <div className="flex gap-2">
                                                    <input className="w-20 border p-1 rounded text-sm" type="number" value={editAmount} onChange={(e) => setEditAmount(e.target.value)} />
                                                    <select className="flex-1 border p-1 rounded text-xs" value={editPayer} onChange={(e) => setEditPayer(e.target.value)}>
                                                        {DEFAULT_PAYERS.map(p => <option key={p} value={p}>{p}</option>)}
                                                    </select>
                                                </div>
                                            </div>
                                            <div className="flex flex-col gap-2">
                                                <button onClick={() => saveEdit(ex.id)} className="bg-green-500 text-white p-1.5 rounded-full"><Check size={14}/></button>
                                                <button onClick={() => deleteExpense(ex.id)} className="bg-red-500 text-white p-1.5 rounded-full"><Trash2 size={14}/></button>
                                            </div>
                                        </div>
                                    ) : (
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    ...
                                                    <span className="text-[10px] text-[#717171] flex items-center gap-1">
                                                        {ex.method === 'Credit Card' ? <CreditCard size={10}/> : <Banknote size={10}/>} {ex.method}
                                                    </span>
                                                </div>
                                            </div>
    ...
    return (
        <div className="h-full px-6 pt-6 pb-24 overflow-y-auto bg-[#F7F7F7]">
            <div className="bg-white p-6 rounded-2xl border border-[#DDDDDD] shadow-sm mb-6 text-center">
                <h3 className="font-extrabold text-[#222222] text-lg mb-2">Google Translate</h3>
                <p className="text-sm text-[#717171] mb-4">Tap below for accurate translation</p>
                <div className="grid grid-cols-2 gap-4">
                    <button onClick={() => openTranslate('cn')} className="bg-[#F7F7F7] p-4 rounded-xl border border-[#DDDDDD] hover:bg-[#FF5A5F] hover:text-white hover:border-[#FF5A5F] transition-all flex flex-col items-center gap-2">
                        <Languages size={24}/>
                        <span className="font-bold text-sm">CN ➔ JP</span>
                    </button>
                    <button onClick={() => openTranslate('en')} className="bg-[#F7F7F7] p-4 rounded-xl border border-[#DDDDDD] hover:bg-blue-500 hover:text-white hover:border-blue-500 transition-all flex flex-col items-center gap-2">
                        <Languages size={24}/>
                        <span className="font-bold text-sm">EN ➔ JP</span>
                    </button>
                </div>
            </div>
            <h3 className="text-[#222222] font-extrabold text-xl mb-4">Saved Phrases</h3>
            <div className="grid gap-4">
                {WINTER_PHRASES.map((p, i) => (
                    <div key={i} onClick={() => setRevealed(revealed === i ? null : i)} className={`bg-white p-5 rounded-2xl border transition-all cursor-pointer ${revealed === i ? 'border-[#FF5A5F] shadow-md ring-1 ring-[#FF5A5F]' : 'border-[#DDDDDD] hover:border-[#222222]'}`}>
                        <div className="flex justify-between items-start">
                            <h3 className="font-bold text-[#222222] text-lg">{p.jp}</h3>
                            <ChevronDown size={20} className={`text-[#717171] transition-transform duration-300 ${revealed === i ? 'rotate-180' : ''}`}/>
                        </div>
                        <p className="text-sm text-[#717171] mt-1">{p.cn}</p>
                        <div className={`grid transition-all duration-300 ${revealed === i ? 'grid-rows-[1fr] opacity-100 mt-4' : 'grid-rows-[0fr] opacity-0'}`}>
                            <div className="overflow-hidden">
                                <div className="bg-[#F7F7F7] p-3 rounded-xl border border-[#DDDDDD]">
                                    <span className="text-[#FF5A5F] font-bold text-sm tracking-wide">{p.romaji}</span>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div className="flex justify-center bg-[#EBEBEB] h-screen font-sans overflow-hidden">
      <div className="w-full max-w-md bg-white h-full relative flex flex-col shadow-2xl overflow-hidden">
        ...
        <nav className="absolute bottom-0 left-0 w-full bg-white border-t border-[#DDDDDD] pb-safe pt-2 px-2 flex justify-around items-center z-50 h-[80px]">
            ...
            <NavButton active={activeTab === 'budget'} onClick={() => setActiveTab('budget')} icon={<div className="w-8 h-8 rounded-full border-2 border-[#222222] flex items-center justify-center"><span className="text-xs font-bold">¥</span></div>} label="Budget" />
            <NavButton active={activeTab === 'reminder'} onClick={() => setActiveTab('reminder')} icon={<CheckSquare size={24} />} label="Reminder" />
            <NavButton active={activeTab === 'translator'} onClick={() => setActiveTab('translator')} icon={<Languages size={24} />} label="Translator" />
        </nav>
      </div>
...
const NavButton = ({ active, onClick, icon, label }) => (
  <button onClick={onClick} className={`flex flex-col items-center justify-center w-16 gap-1 transition-colors ${active ? 'text-[#FF5A5F]' : 'text-[#717171] hover:text-[#222222]'}`}>
    {React.cloneElement(icon, { size: 24, className: active ? 'text-[#FF5A5F]' : 'text-[#717171] hover:text-[#222222]' })}
    <span className={`text-[10px] font-medium ${active ? 'text-[#FF5A5F]' : 'text-[#717171]'}`}>{label}</span>
  </button>
);
